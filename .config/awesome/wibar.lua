...
awful.screen.connect_for_each_screen(function(s)
	...
	awful.tag.find_by_name(s, "4").is_parent = true
...
local function checkWibarForTag(t)
	...
	-- HACK: using a parent tag kinda works for now
	local parent_tag = nil
	for k, st in pairs(selected_tags) do
		if st.index ~= t.index and st.is_parent == true then
			parent_tag = st
		end
	end
	...
	t.screen.mywibox.visible = parent_tag.barvisible
end
